...
## Wave 5 — core diagnostic parity (from T15's survey; 5a ✅ + 5b ✅ done — full sweep after 5b: **loose 71.1%, strict 36.5%, crashes 3**; T21 ✅ done — types strict 235/842)
...
| T21 ✅ | Generic call/construct-signature assignability (deep variance) | L | ✅ Done. New `signature_relation.mbt` implements tsc's signaturesRelatedTo: 1×1 signatures compare strictly (source type params instantiated by structural inference from target params, constraint fallback when the inferred type fails its constraint; target type params alpha-renamed to opaque markers registered in `signature_relation_markers` and given tsc type-param semantics in `is_assignable`); overloaded lists compare with erased type params. Representation: `new (…) => T` constructor-type annotations and generic `<T>(…) => R` function-type annotations now resolve for real (own type params registered in type_cache during resolution — previously every `T` silently became `any`) as Object-with-construct/call-signature (same shape as generic function declarations); Object↔Object relation now requires signature lists to match; Function↔Object arms added; class values now carry one construct signature per constructor overload (was: all overload params concatenated into one bogus signature). Nominal class-parent registry (`class_parent_registry`, populated in `check_class_declaration`) makes constraint/shape checks strict where the TypeReference relation defers. types strict 226→**235**/842 (all 6 assignmentCompatWith{Call,Construct}Signatures targets + both generic ones + stringLiteralTypesOverloadAssignability02), **zero regressions, zero new extras on passing tests**. 27 new tests (assignability_t21_test.mbt); suite **5,346/5,346**. Deferred: (a) inference is first-candidate-wins (no candidate union/priorities) — sufficient for the conformance families, not general unification; (b) marker/constraint checks are nominal-or-lenient for class refs (the checker-free relation can't resolve classes structurally), so some per-line constraint violations tsc reports are silently accepted (code-sets still match); (c) optional params don't widen to `\| undefined` in signature comparison; (d) pre-existing literal-widening of call args makes specialized overloads pick the string signature (typesWithSpecializedConstructSignatures extra TS2322 — also repros with plain function overloads, not a relation bug); (e) call-arg checks against construct-sig params that mention an enclosing generic fn's uninferred type params now emit extra TS2769/TS2345 on 4 already-failing genericCall* tests — needs call-site instantiation before comparison; (f) assignmentCompatWithGenericCallSignatures3 blocked by parser (`=> { <U>(…): U }` type-literal return position, TS1128). | `signature_relation.mbt`, `checker.mbt` |
