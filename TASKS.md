...
## Wave 5 — core diagnostic parity (planned, from T15's survey; sequential-ish, all in checker territory)

| ID | Task | Size | Unlocks | Entry point |
|----|------|------|---------|-------------|
| T16 | TS2304 for unresolved type-reference names in type position (currently silently becomes `any`) — must pre-verify lib globals + in-scope type params to avoid false positives | S/M | ~20–40 tests + removes `any`-cascade extras | `checker.mbt:1695` (`resolve_type_reference` silent fallback) |
| T17 | TS2339 property access on union (must exist on every constituent) & intersection (any constituent, with never-reduction) | M | ~10–15 tests | `checker.mbt` ~10022–10050 |
| T18 | Class-body methods as call signatures in structural assignability (interface methods compare, class methods don't) | M | ~8–12 tests (derivedClassTransitivity*) | class-type member collection feeding relate |
| T19 | Assignability grab-bag: strictNullChecks undefined/null exclusion; enum assignability rules; wrapper-object→primitive rejection (`Boolean`→`boolean`) | M | ~18 tests | core `is_assignable` relation |
| T20 | Tuple & array-literal contextual assignability (array→tuple length rules; binding patterns; for-of/generators cluster) | M/L | ~20–30 tests (partial — many also miss TS2345) | relation + contextual typing; split 4a/4b |
