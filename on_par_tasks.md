...
|----|------|---------|------|--------------------|-------------|
| F1 ✅ | **Real standard library.** ✅ Done (commit 390a394): `tools/gen_embedded_libs.py` embeds 87 JS libs (es5…es2025, esnext.\*) into `lib_sources_gen.mbt`; `lib_loader.mbt` lazily parses per-process following `/// <reference lib>` chains. Checker resolves globals through loaded libs; TS2583/TS2585/TS2318 per tsc baselines; whitelist kept only as DOM fallback; `@lib`/`@noLib` + `--lib`/`--noLib` wired. 23 new tests, suite 5,525/5,525; strict spot-checks: types +29, es6 +9, async +9, zero new strict failures; +0ms when libs untouched, ~45–83ms first consultation. **Deferred**: structural types for lib globals (values resolve as `any` — unblocks D4/D7/Y1 fully when done), DOM/webworker embedding, tsconfig `lib` array, es2021+ Target-enum feature gates, text-scan supplement removal (needs parser support for literal members / type predicates in fn-type params / `abstract new`). | W6 | L | Large: es6 (398 fails), async (125), generators, es2015+ categories all consult globals; also kills a chunk of extra TS2304/2552 (365 tests combined). | `compiler/lib_loader.mbt`, `checker.mbt` |
